...
pub fn router(state: ApiState) -> Router {
    Router::new()
        ...
        .route("/tandem/objects/{object_key}", get(object))
        .route("/tandem/carriers/{txid}/{vout}", get(carrier))
        .route("/tandem/events", get(events))
        .route("/tandem/invalid", get(invalid_events))
        .route("/tandem/reorgs", get(reorgs))
        .route("/tandem/stats", get(stats))
        .route("/tandem/mempool", get(mempool))
        .route("/tandem/agreement/{height}", get(agreement))
        .with_state(state)
}
